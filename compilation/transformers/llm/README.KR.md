...
이 튜토리얼은 Mobilint qb 컴파일러를 사용하여 대규모 언어 모델을 컴파일하는 방법에 대한 상세한 지침을 제공합니다. 컴파일 과정은 표준 트랜스포머 모델을 Mobilint NPU 하드웨어에서 효율적으로 실행할 수 있는 최적화된 `.mxq` 형식으로 변환합니다.
...
- `--model-tag`: 모델 식별자 (출력 디렉토리 이름 지정에 사용)
- `--embedding-path`: Step 1에서 추출한 임베딩 가중치 경로
- `--tokenizer-path`: Hugging Face 토크나이저 식별자
- `--output-dir`: 캘리브레이션 데이터의 기본 디렉토리
- `--min-seqlen`: 최소 시퀀스 길이 (이보다 짧은 샘플은 제외)
...
- `--model-path`: Hugging Face 모델 식별자
...
qb SDK 1.0.0 출시 이후, 모델에 대한 고급 양자화 기술을 지원합니다. 이를 통해 유연한 비트 할당과 고급 양자화 매개변수 최적화를 사용하여 더 나은 성능을 얻을 수 있습니다.
