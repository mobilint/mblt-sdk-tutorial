...
이 튜토리얼은 Mobilint qubee 컴파일러를 사용하여 Whisper 음성-텍스트 변환 모델을 컴파일하는 방법을 상세히 설명합니다. 컴파일 과정은 Whisper 모델(인코더와 디코더를 각각)을 Mobilint NPU 하드웨어에서 효율적으로 실행할 수 있는 최적화된 `.mxq` 형식으로 변환합니다.
...
- Python 3.8 이상
- CUDA 지원 GPU (캘리브레이션 및 컴파일에 필요)
- 충분한 디스크 공간 (모델 + 캘리브레이션 데이터 약 10GB)

### qubee 컴파일러 설치

Mobilint 릴리스 페이지에서 qubee 컴파일러 (버전 0.11.0.1)를 다운로드하여 설치하세요:

1. [https://dl.mobilint.com/releases?series-id=1](https://dl.mobilint.com/releases?series-id=1) 접속
2. qubee 버전 **0.11.0.1**에 해당하는 `.whl` 파일 다운로드
3. wheel 파일 설치:
...
> **디코더 컴파일을 위한 중요 사항:**
>
> qubee==0.11.0.1 설치 후, 디코더 컴파일이 올바르게 작동하려면 약간의 수정이 필요합니다:
>
> 1. 다음 Python 코드를 실행하여 parser 파일 위치를 확인하세요:
>    ```python
>    import qubee
>    print(qubee.model_dict.parser.parser.__file__)
>    ```
>
> 2. 위에서 출력된 파일을 엽니다 (`<qubee_location>/model_dict/parser/parser.py`)
>
> 3. **538번째 줄**의 코드를 다음과 같이 수정하세요:
>    - **변경 전:** `if self.backend == "hf":`
>    - **변경 후:** `if False:`
>
> 이 수정은 Whisper 디코더 컴파일에 필요합니다.
...
먼저 Google FLEURS 데이터셋에서 오디오 데이터를 다운로드합니다. 이 데이터는 캘리브레이션에 사용될 다국어 오디오 샘플을 포함합니다.
...
**수행 내용:**
- 17개 언어에 대해 Google/FLEURS 데이터셋에서 오디오 샘플 다운로드
- 전사 및 번역 생성을 위해 Whisper Large V3 로드
- 16kHz 모노 WAV 형식으로 리샘플링된 오디오 파일 저장
- 원본 언어를 지정하여 Whisper로 전사 생성
- Whisper의 번역 작업을 사용하여 영어 번역 생성

**지원 언어:**
- 아랍어, 중국어(만다린), 독일어, 그리스어, 영어, 스페인어, 프랑스어
...
- whisper-small을 사용하여 실시간으로 전사 및 번역 생성
- 적절한 특수 토큰과 함께 전사 및 번역 작업 모두 지원
- 다양한 캘리브레이션을 위해 전사(80%)와 번역(20%) 작업을 무작위로 혼합

**인코더 캘리브레이션:**
- HuggingFace Whisper 프로세서를 통해 오디오 파일 처리
- `[1, 80, 3000]` 형태의 멜 스펙트로그램 특징 추출 (저장 시 `[1, 3000, 80]`으로 전치)
- `.npy` 형식으로 캘리브레이션 파일 저장

**디코더 캘리브레이션:**
- 인코더를 통해 오디오를 처리하여 은닉 상태 획득
- whisper-small 모델을 사용하여 전사 및 번역 생성
- HuggingFace 프로세서를 사용하여 텍스트 정규화 적용
- 적절한 특수 토큰(SOT, 언어, 작업, 타임스탬프)으로 토큰 시퀀스 생성
- 디코더 캘리브레이션용 입력 임베딩(토큰 임베딩 + 위치 임베딩) 생성
- 인코더 및 디코더 은닉 상태 모두 저장
...
캘리브레이션 데이터를 사용하여 인코더와 디코더를 모두 `.mxq` 형식으로 컴파일합니다.
...
**필수 패키지:**
- transformers==4.50.0
- qubee==0.11.0.1 (사전 요구사항에 설명된 수정 적용 필요)
...
- `global4` 추론 스킴을 사용하여 최종 `.mxq` 형식으로 컴파일
...
> **중요:** 디코더 컴파일을 실행하기 전에, 사전 요구사항 섹션에 설명된 대로 qubee의 parser.py에 수정을 적용했는지 확인하세요.
...
- 전체 시퀀스 길이 캘리브레이션(`use_full_seq_len_calib=True`)과 함께 디코더 캘리브레이션 데이터 사용
...
### 컴파일된 모델
- `compilation/compiled/whisper-small_encoder.mxq` - NPU용 양자화 인코더
- `compilation/compiled/whisper-small_decoder.mxq` - NPU용 양자화 디코더

## 문제 해결

### 디코더 컴파일 실패

디코더 컴파일이 실패하면, qubee의 parser.py에 수정을 적용했는지 확인하세요:
...
2. 해당 파일의 538번째 줄 편집:
   - `if self.backend == "hf":`를 `if False:`로 변경

### 메모리 부족 오류

- 충분한 VRAM을 가진 GPU가 있는지 확인 (8GB 이상 권장)
- 다른 GPU 집약적인 응용 프로그램 종료
- 필요시 캘리브레이션 샘플 수 감소
...
- FLEURS 데이터셋 다운로드를 위한 안정적인 인터넷 연결 확보
