...
def create_encoder_calibration_data(
    audio_dir, output_dir="./encoder", num_samples=1000
):
...
def create_decoder_calibration_data(
    audio_dir, output_dir="./decoder", num_samples=1000
):
    ...
    for i, audio_file in enumerate(audio_files):
        ...
        try:
            ...
            if lang_code is None:
                print(
                    f"  ⚠️ Skipping {audio_file}: language not in FLEURS_TO_WHISPER mapping"
                )
                continue
            ...
            with torch.no_grad():
                transcription_ids = model.generate(
                    inputs.input_features.cuda(), language=lang_code, task="transcribe"
                )
            ...
            with torch.no_grad():
                translation_ids = model.generate(
                    inputs.input_features.cuda(), language=lang_code, task="translate"
                )
            ...
            target_text = (
                translation if use_translation and translation else transcription
            )
            ...
            if use_translation:
                ...
                task_type = (
                    "transcription_no_lang" if False else "transcription_with_lang"
                )
    ...
    calib_json = {
        "info": {
            ...
            "task_types": [
                "transcription_with_lang",
                "transcription_no_lang",
                "translation_with_lang",
                "translation_no_lang",
            ],
...
def main():
    ...
    print(
        "🎯 Creating Calibration Data for Whisper Encoder and Decoder (HuggingFace V3)"
    )
    print("=" * 70)
