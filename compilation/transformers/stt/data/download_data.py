...
def download_fleurs_data(
    output_dir=".", languages=FLEURS_LANGUAGES, num_samples_per_lang=20
):
    ...
    for lang in languages:
        ...
        try:
            ...
            for i in range(min(num_samples_per_lang, len(dataset))):
                ...
                try:
                    print(
                        f"  Transcribing {audio_filename} in {lang} ({source_whisper_lang})..."
                    )
                    result = model.transcribe(
                        audio_path, task="transcribe", language=source_whisper_lang
                    )
                ...
                try:
                    print(
                        f"  Translating {audio_filename} from {lang} ({source_whisper_lang}) to English..."
                    )

                    # Whisper only supports translation to English
                    # Use translate task with source language specified
                    result = model.transcribe(
                        audio_path, task="translate", language=source_whisper_lang
                    )
            ...
            print(
                f"✅ Downloaded {min(num_samples_per_lang, len(dataset))} samples for {lang}"
            )
