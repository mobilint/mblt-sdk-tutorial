...
async def main():
    ...
    dataset = load_dataset("ChristophSchuhmann/MS_COCO_2017_URL_TEXT", split="train")["URL"]

    os.makedirs("coco-selected", exist_ok=True)

    # 2. Select URLs
    idx = random.sample(range(len(dataset)), 100)  # Change 100 to 10000 to see real speed!
    ...
    print(f"Done! Successfully downloaded {sum(results)} out of {len(target_urls)} images.")
