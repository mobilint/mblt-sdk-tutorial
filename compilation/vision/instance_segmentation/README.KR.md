...
본 튜토리얼은 Mobilint `qbcompiler`를 사용하여 인스턴스 분할(Instance Segmentation) 모델을 컴파일하는 상세 가이드를 제공합니다.

여기에서는 Ultralytics에서 제공하는 COCO 데이터셋 사전 학습 모델인 [YOLO11m-seg](https://docs.ultralytics.com/models/yolo11/)를 사용합니다. 이 모델은 이미지 내 객체를 탐지하고 각 객체의 마스크를 생성하는 인스턴스 분할 기능을 수행합니다.
...
캘리브레이션 데이터셋은 모델의 실제 입력 분포를 대표하는 이미지들로 구성됩니다. YOLO11m-seg 모델은 [COCO 데이터셋](https://cocodataset.org/#download)으로 학습되었으므로, COCO 샘플을 사용하여 캘리브레이션을 진행합니다.
...
**파라미터:**
양자화 과정에서 정밀도를 조절하기 위해 다음 파라미터가 사용됩니다:
...
**실행 예시:**
```bash
python model_compile.py --onnx-path ./yolo11m-seg.onnx --calib-data-path ./coco-selected --save-path ./yolo11m-seg.mxq --quant-percentile 0.999 --topk-ratio 0.01 --inference-scheme single
```

위 명령어를 실행하면 현재 디렉토리에 `yolo11m-seg.mxq` 파일이 생성됩니다.
