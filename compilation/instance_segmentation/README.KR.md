...
이 스크립트는 두 REGULUS 세대를 모두 대상으로 합니다. regulus1은 YOLOv9 이하만 지원하고, regulus2는 ARIES와 동일한 모델 범위를 지원합니다. 본 튜토리얼은 YOLOv8 모델을 사용합니다. `--target-device`로 디바이스를 선택합니다 (기본값 `regulus2`는 2026.06 릴리즈 이후, `regulus`는 이전 디바이스용).

모델 준비:

```bash
yolo export model=yolov8m-seg.pt format=onnx # YOLOv8m-seg 모델을 ONNX 형식으로 내보내기
```

컴파일:

```bash
python model_compile_regulus.py --onnx-path ./yolov8m-seg.onnx --calib-data-path ./coco-selected --save-path ./yolov8m-seg.mxq --target-device regulus2
```

위의 명령어를 실행한 후, 컴파일된 모델은 현재 디렉토리에 `yolov8m-seg.mxq`로 저장됩니다.
