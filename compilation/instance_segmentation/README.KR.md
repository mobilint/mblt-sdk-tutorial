...
디바이스마다 모델이 다릅니다: 구버전 REGULUS(`regulus-ra`, 2026-06 이전 고객)는 YOLOv9 이하만 지원하므로 YOLOv8 seg 모델을 사용하고, ARIES(`aries-rb`)와 신버전 REGULUS(`regulus-rb`, 2026-06 이후 고객)는 YOLO11 seg 모델을 사용합니다.
...
디바이스에 맞는 모델을 먼저 export 합니다 (1단계는 `yolo11m-seg` 기준이며, `regulus-ra` 는 `yolov8m-seg` 를 export):

```bash
# YOLO11 seg (aries-rb / regulus-rb 용)
yolo export model=yolo11m-seg.pt format=onnx
# YOLOv8 seg (regulus-ra 용)
