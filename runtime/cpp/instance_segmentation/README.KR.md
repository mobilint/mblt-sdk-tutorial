# 인스턴스 분할 - C++ 추론 (ARIES + REGULUS)

단일 이미지에 대해 C++ NPU 추론을 실행하고 인스턴스 마스크와 bounding box 를
시각화하는 예제입니다. ultralytics v8/v11 의 anchor-free Segment head 가 동일한 출력
layout (3 stride x [reg_max*4 box + nc cls + 32 mask] 채널과 shape [32, 160, 160] 의
prototype 텐서 하나) 을 갖기 때문에, 같은 binary 가 ARIES 와 REGULUS 의 YOLO11m-seg
MXQ 를 모두 처리합니다.
...
- `infer_seg.cc` - 추론 바이너리 소스 (NPU 추론, 후처리, 마스크 + bbox 시각화)
- `yolo_seg_config.h` - 앵커리스 YOLO segment 설정 (yolo11m-seg / yolov8m-seg P5, 80 클래스, 마스크 계수 32 개)
- `utils/` - 공용 추론 모듈 (NPURunner, Transformer, YoloSegDecoder)
- `CMakeLists.txt` - CMake 빌드 설정 (호스트 arch 자동 감지)

## 사전 준비

- [컴파일러 튜토리얼](../../../compilation/instance_segmentation/README.KR.md) 결과의 MXQ 파일이 필요합니다:
  - **ARIES**: `yolo11m-seg.mxq` (`model_compile.py` 산출물).
  - **REGULUS**: `yolov8m-seg.mxq` (`model_compile_regulus.py` 산출물).
...
빌드가 끝나면 `build/infer-seg` 바이너리가 생성됩니다.
...
`build/infer-seg`, `yolov8m-seg.mxq`, `../rc/cr7.jpg` 를 타겟 보드로 복사한 뒤 실행합니다:
...
결과 이미지 `result.jpg` 에는 원본 위에 인스턴스별 분할 마스크(클래스 색상으로 alpha
블렌딩) 와 bounding box, 클래스 라벨이 그려져 저장됩니다.
