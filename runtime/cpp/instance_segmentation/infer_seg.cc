...
static const std::vector<cv::Scalar> COCO_PALETTE = {
    {60, 20, 220},   {32, 11, 119},   {142, 0, 0},     {230, 0, 0},     {228, 0, 106},   {100, 60, 0},
    {100, 80, 0},    {70, 0, 0},      {192, 0, 0},     {30, 170, 250},  {30, 170, 100},  {0, 220, 220},
    {175, 116, 175}, {30, 0, 250},    {42, 42, 165},   {255, 77, 255},  {252, 226, 0},   {255, 182, 182},
    {0, 82, 0},      {157, 166, 120}, {0, 76, 110},    {255, 57, 174},  {0, 100, 199},   {118, 0, 72},
    {240, 179, 255}, {92, 125, 0},    {151, 0, 209},   {182, 208, 188}, {176, 220, 0},   {164, 99, 255},
    {73, 0, 92},     {255, 129, 133}, {255, 180, 78},  {0, 228, 0},     {243, 255, 174}, {255, 89, 45},
    {103, 134, 134}, {174, 148, 145}, {186, 208, 255}, {255, 226, 197}, {1, 134, 171},   {54, 63, 109},
    {255, 138, 207}, {95, 0, 151},    {61, 80, 9},     {51, 105, 84},   {105, 65, 74},   {102, 196, 166},
    {210, 195, 208}, {65, 109, 255},  {149, 143, 0},   {194, 0, 179},   {106, 99, 209},  {0, 121, 5},
    {205, 255, 227}, {208, 186, 147}, {1, 69, 153},    {161, 95, 3},    {0, 255, 163},   {170, 0, 119},
    {199, 182, 0},   {120, 165, 0},   {88, 130, 183},  {0, 32, 95},     {135, 114, 130}, {133, 129, 110},
    {118, 74, 166},  {185, 142, 219}, {114, 210, 79},  {62, 90, 178},   {15, 70, 65},    {115, 167, 127},
    {106, 105, 59},  {45, 108, 142},  {0, 172, 196},   {80, 54, 95},    {255, 76, 128},  {1, 57, 201},
    {122, 0, 246},   {208, 162, 191},
};
...
static void draw_masks(cv::Mat& img, const std::vector<cv::Mat>& masks,
                       const std::vector<YoloSegDecoder::Detection>& dets, float alpha = 0.3f) {
  for (size_t i = 0; i < masks.size(); ++i) {
    ...
    for (int y = 0; y < img.rows; ++y) {
      ...
      for (int x = 0; x < img.cols; ++x) {
        ...
        for (int c = 0; c < 3; ++c) {
          float blended = irow[x][c] * (1.0f - alpha) + static_cast<float>(color[c]) * alpha;
...
static void draw_boxes(cv::Mat& img, const std::vector<YoloSegDecoder::Detection>& dets) {
  for (const auto& d : dets) {
    ...
    const std::string& name =
        (d.cls >= 0 && d.cls < static_cast<int>(COCO_LABELS.size())) ? COCO_LABELS[d.cls] : COCO_LABELS[0];
    ...
    cv::putText(img, label, cv::Point(x1, y1 - 5), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
...
int main(int argc, char** argv) {
  ...
  YoloSegDecoder decoder(nc, nl, IMG_SIZE, reg_max, num_mask_coeffs, conf_thres, iou_thres);
  ...
  auto masks = decoder.assemble_masks(dets, proto, proto_c, proto_h, proto_w, img_h, img_w);
