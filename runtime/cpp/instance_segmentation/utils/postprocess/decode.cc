...
YoloSegDecoder::YoloSegDecoder(int nc, int nl, int img_size, int reg_max, int num_mask_coeffs, float conf_thres,
                               float iou_thres, int max_det)
    : nc_(nc),
      nl_(nl),
      img_size_(img_size),
      reg_max_(reg_max),
      num_mask_coeffs_(num_mask_coeffs),
...
std::vector<YoloSegDecoder::Detection> YoloSegDecoder::decode(const std::vector<mobilint::NDArray<float>>& raw_outputs,
                                                              std::vector<float>& proto_out, int& proto_c, int& proto_h,
                                                              int& proto_w) const {
  ...
  for (const auto& t : raw_outputs) {
    ...
    if (num_mask_coeffs_ > 0 && n % static_cast<size_t>(num_mask_coeffs_) == 0 && static_cast<int>(n) > proto_numel) {
      proto_src = &t;
  ...
  if (det_tensors.size() != cls_tensors.size() || det_tensors.size() != ext_tensors.size()) {
    throw std::runtime_error("decode: det/cls/ext tensor count mismatch (det=" + std::to_string(det_tensors.size()) +
                             " cls=" + std::to_string(cls_tensors.size()) +
                             " ext=" + std::to_string(ext_tensors.size()) + ")");
  }
  ...
  if (anchor_idx != total_anchors) {
    throw std::runtime_error("YoloSegDecoder::decode: NPU outputs do not cover all anchors (" +
  ...
  auto iou_xyxy = [](float ax1, float ay1, float ax2, float ay2, float bx1, float by1, float bx2, float by2) -> float {
...
std::vector<cv::Mat> YoloSegDecoder::assemble_masks(const std::vector<Detection>& dets, const std::vector<float>& proto,
                                                    int proto_c, int proto_h, int proto_w, int orig_h,
                                                    int orig_w) const {
  ...
  // Letterbox geometry: same ratio/padding the Preprocessor used to fit the image into img_size.
  float r = std::min(static_cast<float>(img_size_) / orig_h, static_cast<float>(img_size_) / orig_w);
  ...
  cv::Mat proto_mat(proto_hw, proto_c, CV_32F, const_cast<float*>(proto.data()));

  for (const auto& d : dets) {
    // mask_lin = proto (proto_hw x proto_c) * coeff (proto_c x 1) -> (proto_hw x 1).
    cv::Mat coeff(proto_c, 1, CV_32F, const_cast<float*>(d.mask_coeffs.data()));
    ...
    cv::Rect roi(pad_left, pad_top, std::min(crop_w, img_size_ - pad_left), std::min(crop_h, img_size_ - pad_top));
...
void YoloSegDecoder::scale_to_original(std::vector<Detection>& dets, int img_size, int orig_h, int orig_w) {
