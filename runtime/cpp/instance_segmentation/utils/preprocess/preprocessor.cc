...
void Preprocessor::resize_short_edge(cv::Mat& img, int short_edge, const std::string& interpolation) {
  int h = img.rows;
  int w = img.cols;
...
void Preprocessor::letter_box(cv::Mat& img, cv::Size size) {
  ...
  float ratio = std::min(static_cast<float>(size.height) / h, static_cast<float>(size.width) / w);
...
std::unique_ptr<float[]> Preprocessor::operator()(const cv::Mat& input, const ModelInfo& cfg) {
...
std::unique_ptr<uint8_t[]> Preprocessor::transform_uint8(const cv::Mat& input, const ModelInfo& cfg) {
  cv::Mat img = input.clone();
  for (const auto& p : cfg.m_preprocess_list) {
...
std::unique_ptr<float[]> Preprocessor::transform_float(const cv::Mat& input, const ModelInfo& cfg) {
  cv::Mat img = input.clone();
  for (const auto& p : cfg.m_preprocess_list) {
    if (p.op == PreProcessOps::YOLO) {
      if (std::holds_alternative<std::pair<int, int>>(p.img_size)) {
...
std::unique_ptr<uint8_t[]> Preprocessor::transform_uint8_chw(const cv::Mat& input, const ModelInfo& cfg) {
  cv::Mat img = input.clone();
  for (const auto& p : cfg.m_preprocess_list) {
    if (p.op == PreProcessOps::YOLO) {
      if (std::holds_alternative<std::pair<int, int>>(p.img_size)) {
  ...
  for (int ch = 0; ch < c; ++ch)
    for (int i = 0; i < h * w; ++i) out[ch * h * w + i] = src[i * c + ch];  // HWC -> CHW (channel-planar)
  return out;
}

std::unique_ptr<float[]> Preprocessor::transform_float_chw(const cv::Mat& input, const ModelInfo& cfg) {
  cv::Mat img = input.clone();
  for (const auto& p : cfg.m_preprocess_list) {
    if (p.op == PreProcessOps::YOLO) {
      if (std::holds_alternative<std::pair<int, int>>(p.img_size)) {
