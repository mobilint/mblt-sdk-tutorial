...
// Input mode via --input-dtype: uint8 feeds the fused-normalization MXQ; float feeds a normalized float tensor to the
// !uint8 MXQ.
//
// Usage:
...
int main(int argc, char** argv) {
  ...
  if (pos.size() != 3 || (input_type != "uint8" && input_type != "float")) {
    std::cerr << "Usage: " << argv[0] << " <model.mxq> <image_path> <labels_file> [--input-dtype uint8|float]\n";
    return 1;
  }
