...
public:
    ...
    // HWC (interleaved) counterparts of the above, for the --inf-func hwc diagnostic path
    // (feed via Model::infer). Kept alongside the CHW variants to compare the two APIs.
    std::unique_ptr<uint8_t[]> transform_uint8_hwc(const cv::Mat& input, const ModelInfo& cfg);
    std::unique_ptr<float[]> transform_float_hwc(const cv::Mat& input, const ModelInfo& cfg);
