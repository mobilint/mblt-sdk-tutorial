...
int main(int argc, char** argv) {
  ...
  std::cout << "Inference time: " << std::chrono::duration<double, std::milli>(t1 - t0).count() << " ms\n";

  // DFL decode + NMS, then rescale boxes from letterbox space to original image coordinates.
  YoloDecoder decoder(cfg.m_postprocess.num_classes, cfg.m_postprocess.num_layers, IMG_SIZE, cfg.m_postprocess.reg_max,
