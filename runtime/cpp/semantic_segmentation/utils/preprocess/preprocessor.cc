...
PreprocessedImage Preprocessor::transform_uint8(const cv::Mat& input,
                                                int input_height,
                                                int input_width,
                                                bool channel_last) {
    if (input.empty()) {
    ...
    const float scale =
        std::min(static_cast<float>(input_height) / input.rows,
                 static_cast<float>(input_width) / input.cols);
    ...
    if (input.cols != resized_width || input.rows != resized_height) {
        cv::resize(input, image, cv::Size(resized_width, resized_height), 0, 0,
                   cv::INTER_LINEAR);
    } else {
        image = input.clone();
    }
    ...
    result.letterbox =
        {input_height, input_width, top, bottom, left, right};
    result.data.resize(
        static_cast<size_t>(input_height) * input_width * 3);
    if (channel_last) {
        ...
        for (int channel = 0; channel < 3; ++channel) {
            for (int index = 0; index < pixels; ++index) {
                result.data[channel * pixels + index] =
                    source[index * 3 + channel];
            }
        }
    }
    return result;
