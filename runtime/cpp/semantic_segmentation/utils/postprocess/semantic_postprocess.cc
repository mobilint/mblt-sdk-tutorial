...
LogitsLayout resolve_logits_layout(const std::vector<int64_t>& shape) {
  ...
  if (shape.size() == 4 && shape[0] == 1 && shape[3] == kNumCityscapesClasses) {
    return {static_cast<int>(shape[1]), static_cast<int>(shape[2]), true};
  }
  if (shape.size() == 4 && shape[0] == 1 && shape[1] == kNumCityscapesClasses) {
    return {static_cast<int>(shape[2]), static_cast<int>(shape[3]), false};
  }
  throw std::invalid_argument("Expected HWC or CHW logits with 19 Cityscapes classes.");
}
...
cv::Mat postprocess_semantic(const mobilint::NDArray<float>& output, const LetterboxInfo& letterbox,
                             cv::Size original_size) {
  const LogitsLayout layout = resolve_logits_layout(output.shape());
  if (layout.height != letterbox.input_height || layout.width != letterbox.input_width) {
    throw std::invalid_argument("MXQ and ONNX semantic output spatial shapes do not match.");
  }
  ...
  for (int pixel = 0; pixel < pixels; ++pixel) {
    ...
    int best_class = 0;
    for (int class_id = 0; class_id < kNumCityscapesClasses; ++class_id) {
      const int index = layout.channel_last ? pixel * kNumCityscapesClasses + class_id : class_id * pixels + pixel;
  ...
  const int cropped_width = letterbox.input_width - letterbox.left - letterbox.right;
  const int cropped_height = letterbox.input_height - letterbox.top - letterbox.bottom;
  if (cropped_width <= 0 || cropped_height <= 0) {
    throw std::invalid_argument("Removing letterbox padding produced an empty class map.");
  }

  const cv::Rect content(letterbox.left, letterbox.top, cropped_width, cropped_height);
  if (content.size() == original_size) {
    return class_map(content).clone();
  }
  cv::Mat restored;
  cv::resize(class_map(content), restored, original_size, 0, 0, cv::INTER_NEAREST);
  return restored;
}

cv::Mat visualize_semantic(const cv::Mat& image_bgr, const cv::Mat& class_map, double alpha) {
  if (image_bgr.empty() || class_map.empty()) {
    throw std::invalid_argument("Image and semantic class map must not be empty.");
  }
  if (image_bgr.size() != class_map.size()) {
    throw std::invalid_argument("Image and semantic class-map sizes must match.");
  }
  if (class_map.type() != CV_8UC1) {
    throw std::invalid_argument("Semantic class map must contain uint8 class IDs.");
  }
  ...
  for (int row = 0; row < class_map.rows; ++row) {
    ...
    for (int column = 0; column < class_map.cols; ++column) {
      ...
      if (class_id >= kCityscapesPaletteBgr.size()) {
        throw std::invalid_argument("Cityscapes class ID must be between 0 and 18.");
      }
