...
YoloPoseDecoder::YoloPoseDecoder(int nc, int nl, int img_size, int reg_max, int num_keypoints, float conf_thres,
...
static void stage_outputs(const std::vector<mobilint::NDArray<float>>& raw, int nc, int reg_max, int num_keypoints,
                          int img_size, const std::vector<int>& strides, std::vector<StagedTensor>& det_tensors,
                          std::vector<StagedTensor>& cls_tensors, std::vector<StagedTensor>& kpt_tensors) {
  ...
  if (det_tensors.size() != cls_tensors.size() || det_tensors.size() != kpt_tensors.size()) {
    throw std::runtime_error("decode: det/cls/kpt tensor count mismatch (det=" + std::to_string(det_tensors.size()) +
                             " cls=" + std::to_string(cls_tensors.size()) +
                             " kpt=" + std::to_string(kpt_tensors.size()) + ")");
  }
}
...
std::vector<YoloPoseDecoder::Detection> YoloPoseDecoder::decode(
  ...
  stage_outputs(raw_outputs, nc_, reg_max_, num_keypoints_, img_size_, strides_, det_tensors, cls_tensors, kpt_tensors);
  ...
  // count (reg_max*4, nc, or num_keypoints*3). Each anchor keeps its bases + spatial index.
  struct AnchorAccess {
    const float* box_base;  // start of the box tensor, HWC (hw x reg_max*4)
    const float* cls_base;  // start of the cls tensor, HWC (hw x nc)
    const float* kpt_base;  // start of the kpt tensor, HWC (hw x num_keypoints*3)
  ...
  for (size_t st = 0; st < det_tensors.size(); ++st) {
    ...
    for (int i = 0; i < det.hw; ++i) {
      access[anchor_idx] = {det.data, cls.data, kpt.data, i};
      ++anchor_idx;
    }
  }
  ...
  if (anchor_idx != total_anchors) {
    throw std::runtime_error("YoloPoseDecoder::decode: NPU outputs do not cover all anchors (" +
  ...
  const int kpt_ch = num_keypoints_ * 3;  // kpt tensor channel count (HWC last dim)
  ...
  for (int a : active) {
    ...
    // HWC: box / cls / kpt channels are contiguous rows at base + local*channels.
    ...
    const float* kpt = acc.kpt_base + static_cast<size_t>(acc.local) * kpt_ch;
    ...
    for (int k = 0; k < num_keypoints_; ++k) {
      float raw_x = kpt[k * 3 + 0];
      float raw_y = kpt[k * 3 + 1];
      float raw_s = kpt[k * 3 + 2];
...
void YoloPoseDecoder::scale_to_original(std::vector<Detection>& dets, int img_size, int orig_h, int orig_w) {
