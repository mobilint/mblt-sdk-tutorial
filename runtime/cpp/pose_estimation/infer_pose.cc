...
static const int LIMB_PALETTE_IDX[19] = {9, 9, 9, 9, 7, 7, 7, 0, 0, 0, 0, 0, 16, 16, 16, 16, 16, 16, 16};

// Palette index per keypoint (mirrors coco.py KEYPOINT_PALLETE).
static const int KEYPOINT_PALETTE_IDX[17] = {16, 16, 16, 16, 16, 0, 0, 0, 0, 0, 0, 9, 9, 9, 9, 9, 9};
...
void draw_poses(cv::Mat& img, const std::vector<YoloPoseDecoder::Detection>& dets) {
  int h = img.rows;
  int w = img.cols;
  ...
  for (const auto& d : dets) {
    ...
    cv::rectangle(img, cv::Point(x1, y1), cv::Point(x2, y2), cv::Scalar(0, 255, 0), tl, cv::LINE_AA);

    std::ostringstream label;
    label << "person: " << std::fixed << std::setprecision(1) << pct << "%";
    cv::putText(img, label.str(), cv::Point(x1, y1 - 2), cv::FONT_HERSHEY_SIMPLEX, tl / 2.0, cv::Scalar(255, 255, 255),
                tf, cv::LINE_AA);
    ...
    for (int k = 0; k < static_cast<int>(d.kpts.size()); ++k) {
      ...
      cv::circle(img, cv::Point(static_cast<int>(kp.x), static_cast<int>(kp.y)), radius,
                 POSE_PALETTE[KEYPOINT_PALETTE_IDX[k]], -1, cv::LINE_AA);
    }
    ...
    for (int i = 0; i < 19; ++i) {
      ...
      if (a < 0 || b < 0 || a >= static_cast<int>(d.kpts.size()) || b >= static_cast<int>(d.kpts.size())) {
        continue;
      }
      ...
      cv::line(img, cv::Point(static_cast<int>(ka.x), static_cast<int>(ka.y)),
               cv::Point(static_cast<int>(kb.x), static_cast<int>(kb.y)), POSE_PALETTE[LIMB_PALETTE_IDX[i]],
               limb_thickness, cv::LINE_AA);
    }
...
int main(int argc, char** argv) {
  ...
  // DFL decode + keypoint decode + NMS, then rescale boxes/keypoints from letterbox space to original image
  // coordinates.
  YoloPoseDecoder decoder(nc, nl, IMG_SIZE, reg_max, num_keypoints, conf_thres, iou_thres);
