...
int main(int argc, char** argv) {
  ...
  model_config.setSingleCoreMode({mobilint::CoreId{mobilint::Cluster::Cluster0, mobilint::Core::Core0}});
  ...
  try {
    ...
    if (input_shapes.size() != 1 || input_shapes[0].size() != 3) {
      throw std::runtime_error("Expected one three-dimensional model input.");
    }
    const std::vector<int64_t> input_shape(input_shapes[0].begin(), input_shapes[0].end());
    const bool channel_last = input_shape.back() == 3;
    const bool channel_first = input_shape.front() == 3;
    if (!channel_last && !channel_first) {
      throw std::runtime_error("Could not determine the model input layout.");
    }
    const int input_height = static_cast<int>(channel_last ? input_shape[0] : input_shape[1]);
    const int input_width = static_cast<int>(channel_last ? input_shape[1] : input_shape[2]);

    std::cout << "Model input shape: [" << input_shape[0] << ", " << input_shape[1] << ", " << input_shape[2] << "]\n";
    std::cout << "Image size: " << image.cols << "x" << image.rows << "\n";

    PreprocessedImage input = Preprocessor::transform_uint8(image, input_height, input_width, channel_last);
    std::vector<mobilint::NDArray<uint8_t>> inputs{mobilint::NDArray<uint8_t>(input.data.data(), input_shape)};

    const auto start = std::chrono::high_resolution_clock::now();
    std::vector<mobilint::NDArray<float>> outputs =
        channel_last ? model->infer(inputs, status) : model->inferCHW(inputs, status);
    const auto end = std::chrono::high_resolution_clock::now();
    if (!status) throw std::runtime_error("NPU inference failed.");
    if (outputs.size() != 1) {
      throw std::runtime_error("Depth estimation expects one output tensor.");
    }
    std::cout << "Inference time: " << std::chrono::duration<double, std::milli>(end - start).count() << " ms\n";

    const auto& output_shape = outputs[0].shape();
    std::cout << "Raw MXQ output shape: [";
    for (size_t index = 0; index < output_shape.size(); ++index) {
      std::cout << output_shape[index] << (index + 1 < output_shape.size() ? ", " : "");
    }
    std::cout << "]\n";

    cv::Mat depth = postprocess_depth(outputs[0], input.letterbox, image.size());
    cv::Mat result = visualize_depth(image, depth);
