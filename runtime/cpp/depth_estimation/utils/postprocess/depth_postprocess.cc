...
cv::Mat postprocess_depth(const mobilint::NDArray<float>& output, const LetterboxInfo& letterbox,
  ...
  cv::Mat onnx_depth;
  cv::resize(raw, onnx_depth, cv::Size(letterbox.input_width, letterbox.input_height), 0, 0, cv::INTER_LINEAR);
  ...
  cv::Mat restored;
  cv::resize(onnx_depth(content), restored, original_size, 0, 0, cv::INTER_LINEAR);
  return restored;
}

cv::Mat visualize_depth(const cv::Mat& image_bgr, const cv::Mat& depth, double alpha) {
  ...
  for (int row = 0; row < depth.rows; ++row) {
    ...
    for (int column = 0; column < depth.cols; ++column) {
      if (valid_row[column]) {
        const float scaled = (disparity_row[column] - lower) * 255.0f / (upper - lower);
        normalized_row[column] = cv::saturate_cast<uint8_t>(std::clamp(scaled, 0.0f, 255.0f));
      }
    }
  }
