...
YOLOv9m MXQ 모델을 사용하여 단일 이미지에 대해 C++ NPU 추론을 실행하고, 바운딩 박스를 시각화하는 예제입니다.
...
빌드가 완료되면 `build/infer-det` 바이너리가 생성됩니다.
...
`build/infer-det` 바이너리와 `yolov9m.mxq` 모델 파일을 타겟 보드에 복사한 뒤 실행합니다:
...
`result.jpg`에 바운딩 박스와 클래스 이름이 그려진 결과 이미지가 저장됩니다.
