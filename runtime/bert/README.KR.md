...
컴파일된 MXQ 모델을 사용하여 Mobilint NPU에서 추론을 실행합니다. 이 스크립트는 `BertMXQModel` 래퍼 클래스(`wrapper/bertmxq.py` 참조)를 사용하여 더미 문장 쌍 간의 코사인 유사도를 계산합니다.
...
래퍼 클래스(`wrapper/bertmxq.py`)는 입력 임베딩(word, token type, position)을 CPU에서 처리하고, 나머지 트랜스포머 레이어는 `qbruntime`을 통해 Mobilint NPU에서 실행합니다.
