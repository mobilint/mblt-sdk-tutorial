...
이 튜토리얼은 Mobilint `qbruntime`를 사용해 컴파일된 얼굴 탐지 MXQ 모델을 실행하는 방법을 설명합니다.

시작하기 전에 [../../../compilation/face_detection/README.KR.md](../../../compilation/face_detection/README.KR.md)의 컴파일 과정을 먼저 완료하세요. 이 디렉토리의 런타임 예제는 컴파일된 모델이 `../../../compilation/face_detection/yolov12m-face.mxq`에 있다고 가정합니다.

전체 구조는 [../object_detection/README.KR.md](../object_detection/README.KR.md)와 비슷하지만, 이 튜토리얼은 단일 클래스 얼굴 탐지기와 전용 후처리 파이프라인을 사용합니다.
...
5. 단일 클래스 얼굴 검출 결과를 디코드하고 NMS를 적용한 뒤, 얼굴 박스와 confidence score를 렌더링합니다.
...
- `postprocess.py`: 얼굴 탐지 출력을 재배열하고 anchorless 예측을 디코드한 뒤 NMS를 적용합니다.
- `visualize.py`: 원본 이미지에 얼굴 바운딩 박스와 confidence score를 그립니다.
- `face_metadata.py`: 단일 `face` 라벨과 시각화 메타데이터를 정의합니다.
...
추론 후에는 필요할 경우 HWC 형태의 NPU 출력을 BCHW로 다시 변환해 `postprocess.py`가 하나의 레이아웃만 처리하도록 맞춥니다. 이후 `YoloPostProcessAnchorless`가 box head와 classification head를 묶고 distribution focal loss (DFL) 기반 예측을 디코드한 뒤 Non-Maximum Suppression (NMS)을 적용합니다.

이 모델은 단일 클래스 모델이므로, 유지된 모든 detection은 `face` 라벨로 표시됩니다.
...
- 모델: `../../../compilation/face_detection/yolov12m-face.mxq`
- 입력 이미지: `../rc/cr7.jpg`
- 출력 이미지: `./tmp/cr_demo.jpg`
...
스크립트는 `tmp/cr_demo.jpg`와 같은 결과 이미지를 저장하며, 원본 이미지 위에 얼굴 바운딩 박스와 confidence score를 그립니다.
