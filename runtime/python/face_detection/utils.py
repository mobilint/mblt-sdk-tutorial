...
# Postprocess runs after NPU inference, so keeping tensors on CPU avoids
# unnecessary host<->device copies for small single-image face detection flows.
...
def make_anchors(imh, imw, strides, grid_cell_offset=0.5):
    ...
    for stride in strides:
        ...
        sx = torch.arange(end=w, dtype=torch.float32, device=DEVICE) + grid_cell_offset  # shift x
        sy = torch.arange(end=h, dtype=torch.float32, device=DEVICE) + grid_cell_offset  # shift y
...
class DFL(nn.Module):
    ...
    def __init__(self, c1=16):
        ...
        x = torch.arange(c1, dtype=torch.float32, device=DEVICE)
