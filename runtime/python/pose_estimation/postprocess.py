...
class YoloPosePostProcessAnchorless(YoloPostProcessAnchorless):
    ...
    def process_box_cls(self, box_cls):
        ...
        if box_cls.numel() == 0:
            return torch.zeros((0, 4 + self.nc + self.n_extra), dtype=torch.float32, device=self.device)  # (0, 56)
        ...
        key_coord = (key_coord * 2 + self.anchors[:, ic] - 0.5) * self.strides[:, ic]  # (1, 17, 2, *)
