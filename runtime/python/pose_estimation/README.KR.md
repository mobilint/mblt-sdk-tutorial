...
이 튜토리얼은 Mobilint `qbruntime`를 사용해 컴파일된 포즈 추정 MXQ 모델을 실행하는 방법을 설명합니다.

시작하기 전에 [../../../compilation/pose_estimation/README.KR.md](../../../compilation/pose_estimation/README.KR.md)의 컴파일 과정을 먼저 완료하세요. 이 디렉토리의 런타임 예제는 컴파일된 모델이 `../../../compilation/pose_estimation/yolo11m-pose.mxq`에 있다고 가정합니다.
...
5. 검출 결과와 keypoint를 디코드하고 NMS를 적용한 뒤, 바운딩 박스와 포즈 skeleton을 렌더링합니다.
...
- `postprocess.py`: YOLO pose 출력을 재배열하고 박스와 keypoint를 디코드한 뒤 NMS를 적용합니다.
- `visualize.py`: 원본 이미지에 바운딩 박스, 라벨, keypoint, skeleton 선을 그립니다.
- `coco.py`: COCO keypoint 메타데이터와 클래스 정보를 제공합니다.
...
추론 후에는 필요할 경우 HWC 형태의 NPU 출력을 BCHW로 다시 변환해 `postprocess.py`가 하나의 레이아웃만 처리하도록 맞춥니다. 이후 후처리 단계에서 바운딩 박스와 keypoint를 디코드하고 confidence threshold와 Non-Maximum Suppression (NMS)을 적용합니다.
...
- 모델: `../../../compilation/pose_estimation/yolo11m-pose.mxq`
- 입력 이미지: `../rc/cr7.jpg`
- 출력 이미지: `./tmp/cr_pose_demo.jpg`
...
스크립트는 `tmp/cr_pose_demo.jpg`와 같은 결과 이미지를 저장하며, 원본 이미지 위에 바운딩 박스, 사람 라벨, keypoint, skeleton 연결을 그립니다.
