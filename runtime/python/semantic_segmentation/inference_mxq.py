"""Run YOLO26 semantic segmentation with a compiled MXQ model."""
...
import numpy as np
import qbruntime
from postprocess import postprocess_semantic
from visualize import save_semantic_overlay
...
def parse_args() -> ArgumentParser:
    parser = ArgumentParser(description="Run semantic segmentation with a compiled MXQ model")
    parser.add_argument(
        "--model-path",
        default="../../../compilation/semantic_segmentation/yolo26m-sem.mxq",
        help="Path to the compiled MXQ model",
    )
    parser.add_argument("--image-path", default="../rc/munster.png", help="Path to the input image")
    parser.add_argument(
        "--output-path",
        default="./tmp/munster_semantic_demo.png",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Segmentation-overlay opacity between 0 and 1",
    )
    return parser
...
def main() -> None:
    ...
    class_map = postprocess_semantic(
        outputs,
    ...
    print(f"Saved the semantic-segmentation visualization to {args.output_path}")
