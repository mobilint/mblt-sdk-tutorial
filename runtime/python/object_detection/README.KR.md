...
이 튜토리얼은 Mobilint `qbruntime`을 사용하여 컴파일된 객체 탐지 모델로 추론을 실행하는 방법에 대한 단계별 지침을 제공합니다.

이 가이드는 [../../../compilation/object_detection/README.KR.md](../../../compilation/object_detection/README.KR.md)에서 이어지는 내용입니다. 모델 컴파일을 성공적으로 마쳤으며 다음 파일이 준비되어 있다고 가정합니다:

- `../../../compilation/object_detection/yolo11m.mxq` - 컴파일된 모델 파일
...
스크립트는 바운딩 박스가 그려진 이미지를 `tmp/cr_demo.jpg`에 저장합니다.
