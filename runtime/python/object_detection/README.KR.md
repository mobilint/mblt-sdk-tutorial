...
이 튜토리얼은 Mobilint `qbruntime`를 사용해 컴파일된 객체 탐지 MXQ 모델을 실행하는 방법을 설명합니다.

시작하기 전에 [../../../compilation/object_detection/README.KR.md](../../../compilation/object_detection/README.KR.md)의 컴파일 과정을 먼저 완료하세요. 이 디렉토리의 런타임 예제는 컴파일된 모델이 `../../../compilation/object_detection/yolo11m.mxq`에 있다고 가정합니다.
...
5. 원시 출력을 디코드하고 confidence filtering 및 NMS를 적용한 뒤, 검출 결과를 이미지에 그립니다.
...
- `postprocess.py`: YOLO 출력을 재배열하고 anchorless 예측을 디코드한 뒤 NMS를 적용합니다.
- `visualize.py`: 원본 이미지에 바운딩 박스와 클래스 라벨을 그립니다.
...
추론 후에는 필요할 경우 HWC 형태의 NPU 출력을 BCHW로 다시 변환해 `postprocess.py`가 하나의 레이아웃만 처리하도록 맞춥니다. 이후 후처리 단계에서 바운딩 박스를 디코드하고 confidence threshold와 Non-Maximum Suppression (NMS)을 적용합니다.
...
- 모델: `../../../compilation/object_detection/yolo11m.mxq`
- 입력 이미지: `../rc/cr7.jpg`
...
스크립트는 `tmp/cr_demo.jpg`와 같은 결과 이미지를 저장하며, 원본 이미지 위에 바운딩 박스와 COCO 클래스 라벨을 그립니다.

후처리 후 검출 결과가 남지 않으면, 스크립트는 원본 이미지를 그대로 출력 경로에 저장합니다.
